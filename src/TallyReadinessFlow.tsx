  ...
  return (
    <section className="tally-readiness" aria-labelledby="tally-readiness-title">
      ...
      <ol className="tally-readiness-steps" aria-label="Tally readiness steps">
        ...
        <ReadinessStep complete={companySaved} number={3} title="Save reviewed scope" detail={companySaved ? "Observed company identity persisted" : "Save only after review"} />
      </ol>
      ...
      <div className="tally-readiness-action">
        ...
        {!endpointComplete ? (
          <button className="primary" type="button" onClick={onCheck} disabled={busy}>
            <Cable size={18} /> {busy ? "Checking local Tally…" : "Check local Tally"}
          </button>
        ) : !companyName ? (
          <a className="secondary-action readiness-link" href="#company-profile"><Building2 size={18} /> Choose company</a>
        ) : companySaved ? (
          <span className="readiness-complete"><Check size={18} /> Company evidence saved</span>
        ) : (
          <a className="secondary-action readiness-link" href="#company-profile"><Building2 size={18} /> Review company</a>
        )}
      </div>
    </section>
  );
