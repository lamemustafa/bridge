-- Scope commitment v2 binds the selected composite company identity. Rebuild
-- the parent and its observation child together: renaming only the parent
-- redirects the child's composite foreign key to the legacy table.
DROP TRIGGER IF EXISTS tally_selected_read_scopes_no_update;
DROP TRIGGER IF EXISTS tally_selected_read_scope_authority_required;
DROP TRIGGER IF EXISTS tally_selected_read_scopes_no_delete;
...
ALTER TABLE tally_selected_read_observations
  RENAME TO tally_selected_read_observations_v23;
ALTER TABLE tally_selected_read_scopes
  RENAME TO tally_selected_read_scopes_v23;

CREATE TABLE tally_selected_read_scopes (
  id TEXT PRIMARY KEY,
  capability_snapshot_id TEXT NOT NULL UNIQUE,
  company_id TEXT NOT NULL,
  scope_contract_version INTEGER NOT NULL CHECK (scope_contract_version IN (1, 2)),
...
SELECT
  ...
  observed_at_unix_ms, completeness_state, no_writes_attempted,
  raw_records_retained
FROM tally_selected_read_scopes_v23;
...
SELECT
  ...
  company_context_verified, schema_verified, record_count_verified,
  identity_evidence_state, date_window_verified
FROM tally_selected_read_observations_v23;

DROP TABLE tally_selected_read_observations_v23;
DROP TABLE tally_selected_read_scopes_v23;

CREATE TRIGGER tally_selected_read_scopes_no_update
BEFORE UPDATE ON tally_selected_read_scopes
BEGIN
  SELECT RAISE(ABORT, 'selected read scopes are immutable');
END;
CREATE TRIGGER tally_selected_read_scope_authority_required
...
END;
CREATE TRIGGER tally_selected_read_scopes_no_delete
...
VALUES (24, 'versioned selected-read scope commitments preserve parent-child evidence', 0);
