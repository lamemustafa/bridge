...
fn captured_wr2_native_core_window_builds_without_report_field_amounts() {
    ...
    assert!(voucher_type_request.contains("<FETCH>NAME, PARENT, GUID, MASTERID, ALTERID</FETCH>"));
...
fn fully_populated_canary_does_not_overclaim_field_support() {
    ...
    let window = CanonicalPackWindow {
        ...
        record_evidence: Some(vec![bridge_tally_core::SourceRecordEvidence {
            ...
            raw_source_sha256: bridge_tally_core::RawSourceSha256::parse("0".repeat(64)).unwrap(),
            alter_id: None,
        }]),
    };
...
async fn snapshot_start_uses_its_fresh_education_profile_before_ledger_export() {
    ...
    let extent = company_extent_with_books_from("Education Mid-month", company_guid, "20240115");
    ...
    let connector = RuntimeTallyConnector::new(runtime.clone(), config.clone(), company, context)
