...
//!
//! Folded from the former standalone `bridge-tally-runtime` crate: it had exactly one consumer
//! (`crate::tally::runtime`), so the crate boundary earned nothing except hiding this module's
//! true dead code from the `dead_code` lint. This module still authenticates no endpoint and
//! establishes no accounting or support claim -- it controls local execution and emits only
//! fixed-cardinality, privacy-reduced observations.
...
use crate::observability::{BodyBytesObservation, CircuitRejectReason};
...
pub(super) struct EndpointIdentity(String);

impl EndpointIdentity {
    pub(super) fn new(value: impl Into<String>) -> Result<Self, RuntimeConfigurationError> {
...
#[cfg_attr(
    ...
    allow(
        dead_code,
        reason = "OtherRead is exercised by tests; no production call site issues it"
    )
)]
...
impl ReadOperation {
    pub(super) const fn request_class(self) -> RequestClass {
        match self {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(super) enum EndpointCircuitState {
    Closed,
    Open,
    HalfOpen,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(super) struct EndpointRuntimeSnapshot {
    pub(super) consecutive_failures: u32,
    pub(super) circuit_state: EndpointCircuitState,
    pub(super) circuit_retry_after_unix_ms: Option<i64>,
    pub(super) half_open_probe_in_flight: bool,
    pub(super) last_failure_unix_ms: Option<i64>,
}
...
pub(super) enum ReadAttempt<T, E> {
    Success {
        value: T,
...
#[cfg_attr(
    not(test),
    allow(dead_code, reason = "constructor and accessor used only by tests")
)]
impl ReadRetryPolicy {
    pub(super) const SINGLE_ATTEMPT: Self = Self {
        maximum_attempts: 1,
    ...
    pub(super) fn transient_default() -> Self {
        Self {
            maximum_attempts: 3,
...
pub(super) struct RuntimePolicy {
    ...
    pub(super) request_spacing: Duration,
    pub(super) circuit_failure_threshold: u32,
    pub(super) circuit_cooldown: Duration,
    pub(super) maximum_endpoint_sessions: usize,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
// `Invalid` postfix is the point: every variant names an invalid input.
#[allow(clippy::enum_variant_names)]
#[cfg_attr(
    not(test),
    allow(
        dead_code,
        reason = "RetryPolicyInvalid is unreachable while retry policies are constructed internally"
    )
)]
pub(super) enum RuntimeConfigurationError {
...
#[derive(Debug)]
pub(super) enum ReadExecutionError<E> {
    QueueDeadline,
    Cancelled,
    CircuitRejected { reason: CircuitRejectReason },
...
#[derive(Clone)]
pub(super) struct PortableReadRuntime {
...
impl PortableReadRuntime {
    pub(super) fn new(policy: RuntimePolicy) -> Result<Self, RuntimeConfigurationError> {
        Self::with_collector(policy, Arc::new(TelemetryCollector::new()))
    }

    pub(super) fn with_collector(
        policy: RuntimePolicy,
        collector: Arc<TelemetryCollector>,
    ) -> Result<Self, RuntimeConfigurationError> {
        Ok(Self {
    ...
    pub(super) fn collector(&self) -> Arc<TelemetryCollector> {
    ...
    pub(super) async fn execute_read<T, E, F, Fut>(
        &self,
        endpoint: EndpointIdentity,
        ...
        Fut: Future<Output = ReadAttempt<T, E>>,
    {
        ...
        let permit = match session.circuit.admit(now_unix_ms()) {
            ...
            Err((reason, _retry_after_unix_ms)) => {
                ...
                return Err(ReadExecutionError::CircuitRejected { reason });
            }
        };
