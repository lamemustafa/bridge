...
fn build_import_guidance(
    ...
    let stale_classification_warning = bank_types.then_some(
        "This file's Payment, Receipt and Contra split came from the group collection read during this build. Regrouping a ledger afterwards is an ordinary Tally operation and would silently make the voucher type wrong — a counterparty moved under a cash or bank group should have become a Contra. post_import classifies every leg again just before it posts and refuses a changed one, but a manual import of this file is not checked, and verify_import compares the entries as built, not current ancestry. If any master changed since this batch was built, discard it and build again.",
    );
