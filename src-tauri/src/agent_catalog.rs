                ...
                let (description, input_schema) = match name {
                    ...
                    "profit_and_loss" | "balance_sheet" => (
                        if name == "profit_and_loss" {
                            "Return the Profit and Loss for a date range, derived from Tally's native Trial Balance: one line per reserved P&L primary group (Sales Accounts, Direct Incomes, Purchase Accounts, Direct Expenses, Indirect Incomes, Indirect Expenses), each the window's signed debit plus credit movement (a debit negative), with `gross_result` and `net_result` (a profit positive). Reads the Trial Balance, the group tree and Tally's own Balance Sheet inside one company, mode and book-extent bracket; requires observed INR currency (a book with more than one currency master is refused) and supported date boundaries. Each line reports `amount` (`sum`, `present_count`, `empty_count`): the sum is over the amounts Tally returned, and the empty ones it left out are counted. A result is established only if every ledger is classified (a ledger under a user-created primary group, or whose group chain is incomplete, is listed in `unclassified`, up to 100, with `unclassified_total`), no Stock-in-Hand ledger carries an amount, and Tally's own Balance Sheet for the window ties line for line to the derived one (`balance_sheet_gate`). Otherwise it is `not_established` with a `reason`; for `tally_balance_sheet_differs`, `lines` names the lines that did not tie. A book with stock items or a foreign-currency difference is expected to refuse; no inventory book has been measured. Tally's own statements carry no company identity and are bound only by the checks around the read. The gate has been measured over one full year on one book and one month on another; a window spanning more than one financial year is unmeasured. Tally's own Profit and Loss is read too and compared by display name in `tie_out` (`matched`, `matched_empty_as_zero`, `differs` or `not_compared`); that comparison gates nothing."
                        } else {
                            "Return the Balance Sheet for a date range, derived from Tally's native Trial Balance: one line per reserved Balance Sheet primary group (Capital Account, Loans (Liability), Current Liabilities, Suspense A/c, Branch / Divisions, Fixed Assets, Investments, Current Assets, Misc. Expenses (ASSET)), each the signed closing balance at `to` (a debit negative), and `result.profit_and_loss`: the Profit & Loss A/c ledger's closing and the `carried` result (that closing plus every P&L ledger's closing; in a part-year window the year's earlier result sits in that ledger). Reads the Trial Balance, the group tree and Tally's own Balance Sheet inside one company, mode and book-extent bracket; requires observed INR currency (a book with more than one currency master is refused) and supported date boundaries. Each line reports `amount` (`sum`, `present_count`, `empty_count`): the sum is over the amounts Tally returned, and the empty ones it left out are counted. A result is established only if every ledger is classified (a ledger under a user-created primary group, or whose group chain is incomplete, is listed in `unclassified`, up to 100, with `unclassified_total`), no Stock-in-Hand ledger carries an amount, and Tally's own Balance Sheet for the window ties line for line to the derived one (`balance_sheet_gate`). Otherwise it is `not_established` with a `reason`; for `tally_balance_sheet_differs`, `lines` names the lines that did not tie. A book with stock items or a foreign-currency difference is expected to refuse; no inventory book has been measured. Tally's own statements carry no company identity and are bound only by the checks around the read. The gate has been measured over one full year on one book and one month on another; a window spanning more than one financial year is unmeasured."
                        },
