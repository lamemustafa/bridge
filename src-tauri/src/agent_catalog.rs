        ...
        names
            ...
            .map(|name| {
                let (description, input_schema) = match name {
                    ...
                    "post_import" => (
                        "Ask the local user to review and approve ONE saved Journal, Payment, Receipt or Contra in a native dialog, then attempt posting once and read it back. Requires opt-in. A Payment, Receipt or Contra is refused (import_bank_classification_changed) if any leg's cash/bank classification changed since the build, checked before approval and again after approval inside the endpoint queue, before the final duplicate check and the post. Every post is aimed by a last all-company snapshot, refused as post_company_scope_changed (or post_company_scope_unconfirmed if unreadable) unless exactly one loaded company has the target's GUID and name and no other loaded company's name could match it; the result's post_location says which companies' voucher marks moved after it. Repeating the original batch only reconciles; never rebuild the same event after a timeout. The model cannot approve it. No master creation, sales, purchase, tax, inventory, alteration or deletion.",
