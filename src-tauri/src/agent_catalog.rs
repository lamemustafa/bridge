        ...
        names
            ...
            .map(|name| {
                let (description, input_schema) = match name {
                    ...
                    "ledger_masters" => (
                        "Return verified ledger masters with monetary fields on a freshly observed supported product/mode; compliance includes paired party-master observations. With fields=compliance, each ledger also carries `ancestry`: its resolved group chain (`chain`, nearest ancestor first, each hop's own `name` and `reserved_name`), `complete` (true only if the chain was resolved all the way to the reserved account root), and `gap` (null when complete, else why resolution stopped: no_parent, group_absent, group_name_repeated, reserved_name_missing, cycle or exhausted). A `reserved_name` beginning with U+FFFD `#4;` is a Tally reserved value (Tally writes it as `&#4;`); `U+FFFD#4; Primary` is the account root, distinct from a group a user named Primary. An incomplete chain is never padded or guessed -- `chain` is exactly what was resolved and no more, and a caller must check `complete` before treating it as exhaustive; this is not available with fields=basic, which never reads the group collection. `group` filters by group name; `group_scope` controls what it matches against -- \"immediate\" (default, unchanged) matches only the ledger's own parent, \"ancestry\" also matches any group in its resolved chain (e.g. a ledger under `Bank OD A/c` matches a `group_scope: ancestry` filter for `Loans (Liability)`) and requires fields=compliance. A gap in a ledger's chain never counts as an ancestry-scope match. Opt-in fields=compliance_diagnostics returns matched rows and unresolved source observations tagged by join_state on a single items pager. It rejects group and group_scope arguments. Check result.state and evidence.state plus global coverage on every page: partial means exact joins remain unresolved, independently of truncation. Unresolved records carry no money or compliance assertion. total counts diagnostic observations, not distinct ledgers. Pages are fresh reads; require matching source evidence commitments before combining them. Existing compliance reads and workbook exports remain complete-only.",
                        json!({"type":"object","additionalProperties":false,"required":["company_guid"],"properties":{"company_guid":{"type":"string","minLength":1},"group":{"type":"string"},"group_scope":{"type":"string","enum":["immediate","ancestry"],"default":"immediate"},"fields":{"type":"string","enum":["basic","compliance","compliance_diagnostics"],"default":"basic"},"offset":{"type":"integer","minimum":0,"default":0},"limit":{"type":"integer","minimum":1,"default":500}}}),
                    ),
                    "ledger_movement" => (
