                ...
                let (description, input_schema) = match name {
                    ...
                    "profit_and_loss" | "balance_sheet" => (
                        if name == "profit_and_loss" {
                            "Return the Profit and Loss for a date range, derived from Tally's native Trial Balance: one line per reserved P&L primary group (Sales Accounts, Direct Incomes, Purchase Accounts, Direct Expenses, Indirect Incomes, Indirect Expenses), each the window's signed debit plus credit movement (a debit negative), with `gross_result` and `net_result` (a profit positive). Reads the Trial Balance, the group tree and Tally's own Profit and Loss inside one company, mode and book-extent bracket; requires observed INR currency and supported date boundaries. Each line reports `amount` (`sum`, `present_count`, `empty_count`): an empty amount is excluded and counted, never read as zero. A ledger under a user-created primary group, or whose group chain is incomplete, is listed in `unclassified` (up to 100, with `unclassified_total`); while any carries an amount, both results are `not_established`. Closing stock is not derived, so with a Stock-in-Hand balance both results are `not_established`. `tie_out` compares Tally's own lines to the derived ones by display name (`matched`, `matched_empty_as_zero`, `differs` or `not_compared`) and enforces nothing."
                        } else {
                            "Return the Balance Sheet for a date range, derived from Tally's native Trial Balance: one line per reserved Balance Sheet primary group (Capital Account, Loans (Liability), Current Liabilities, Suspense A/c, Branch / Divisions, Fixed Assets, Investments, Current Assets, Misc. Expenses (ASSET)), each the signed closing balance at `to` (a debit negative), and `result.profit_and_loss`: the Profit & Loss A/c ledger's closing and the `carried` result (that closing plus every P&L ledger's closing). Reads the Trial Balance, the group tree and Tally's own Balance Sheet inside one company, mode and book-extent bracket; requires observed INR currency and supported date boundaries. Each line reports `amount` (`sum`, `present_count`, `empty_count`): an empty amount is excluded and counted, never read as zero. A ledger under a user-created primary group, or whose group chain is incomplete, is listed in `unclassified` (up to 100, with `unclassified_total`); while any carries an amount, `carried` is `not_established`. `tie_out` compares Tally's own lines to the derived ones by display name (`matched`, `matched_empty_as_zero`, `differs` or `not_compared`) and enforces nothing; Tally's own Profit & Loss line has been compared over one full year on one book and one month on another."
                        },
