...
- **Request:** the text is in `vouchers-forex-composite-20260915.request.xml`. It is byte for byte what `render_agent_vouchers` renders today for this company and window, unnarrowed, with the `AGENT_VOUCHER_FETCH` list.
...
- The response carries no GSTIN, PAN, contact or address field.
