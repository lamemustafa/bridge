...
//!   misstates the walk three times: it starts from the opening Stock Summary's quantity where the
//!   summary lists the item (only the rest from the master's own opening), not "from each item's
//!   own opening quantity"; it walks the population's vouchers (regular ones, with no date bound),
//!   not "every FY inventory voucher"; and a line's direction is the stock journal's IN/OUT tag,
//!   else the sign of the inventory line's own amount (below), not "the ledger-entry sign each
//!   allocation is nested under".
