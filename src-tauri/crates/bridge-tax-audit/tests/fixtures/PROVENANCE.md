...
the same canonical `cash_44ab`, `cash_payments_40a3`, `depreciation` and `financial_statements` results
...
`golden/synthetic.depreciation.json`, `golden/synthetic.financial_statements.json` and
`golden/synthetic.financial_statements.noreport.json` are the reference Python implementation's own canonical dumps
