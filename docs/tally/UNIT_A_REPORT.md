...
| Exact outstandings calculation | Exact-decimal lifecycle tests and real wildcard fixture totals; identical scan at two as-of dates preserves receivable/payable totals and open-bill count while changing ageing buckets; port 9000 matches the native-report target | Proven live on port 9000 only; port 9001 withholds totals by design (ruling 9 §3a) |
...
| Complete-book run on ports 9000 and 9001 | The exact target-bound check admits corpus-bound width 252 and owner-attested Educational boundaries only under non-default `live-calibration-harness`, takes explicit as-of `20260731`, and stays ignored. Default builds have no width/profile constructor. Port 9000 completes at the accepted target; port 9001 correctly returns Partial `ledger_opening_bills_not_covered`, because that restored book carries ten bill-wise ledger openings worth over ₹15 lakh that port 9000 does not | Proven live on port 9000. **Port 9001 does NOT complete, by design** — an earlier revision of this row claimed identical accounting on both ports, which was true only before the divergence was measured (ruling 9 §3a) |
