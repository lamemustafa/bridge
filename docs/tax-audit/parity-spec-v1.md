...
- **Case mapping.** Rust 1.96's `char` tables are Unicode 17.0.0. Measured over every code point,
  ...
  `py_lower` keep those code points unchanged, so they are exact **per code point**. They are not
  exact on every string: lowering a final capital sigma depends on the Cased / Case_Ignorable
  properties, which also moved between the versions and are not pinned (measured by an independent
  review). The lists are generated by `parity/case_exceptions.py`, and a unit test fails when the
...
- **Where it is used.** The narration keys and own-account terms of `cash_book_integrity`, the
  ...
  is a known, measured gap, closed by pinned helpers before any further test uses these idioms.
