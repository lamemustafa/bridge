...
- **Case mapping.** Rust 1.96's `char` tables are Unicode 17.0.0. Measured over every code point,
  ...
  code points; Python maps every one of them to itself. `bridge-tax-audit` routes every case
  mapping that reaches a figure, a match or a key through `support::py_upper` / `py_lower`, which
  keep those code points unchanged. The lists are generated by `parity/case_exceptions.py`, and a
  unit test fails when the toolchain's tables move, so they are regenerated rather than left
  stale.
- **NFC.** The `unicode-normalization` crate and Python 3.13's `unicodedata.normalize("NFC", ...)`
  agree on every single code point (measured). Multi-code-point sequences have not been measured
  exhaustively.
