...
**codes**, e.g. `["ID-1", "MAP-1", "POP-0", "POP-1", "POP-2", "POP-3"]`) and
...
| Report | Source | Codes |
|---|---|---|
| `book_*` | the reference engine's book-level invariant functions, run on the Book | `ID-1`, `POP-0`, `POP-1`, `POP-2`, `POP-3`, `MAP-1` |
