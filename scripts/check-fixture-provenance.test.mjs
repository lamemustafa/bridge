...
test("a path row does not document another fixture with the same basename", async () => {
  ...
  try {
    ...
    const output = runGateExpectingFailure(root);
    assert.match(output, /other\/captured\.bin: named in no provenance record/);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

// A path in prose documents its own file, resolved against the note or the
// repository root, as REOPEN_CAPTURE_PROVENANCE.md and scripts/fixtures do.
test("a path named in prose documents the fixture at that path", async () => {
  const root = await makeTree();
  try {
    const fixtures = join(root, "src-tauri/crates/bridge-tally-protocol/tests/fixtures");
    await mkdir(join(fixtures, "generators"), { recursive: true });
    await writeFile(join(fixtures, "generators", "build.py"), "print()\n");
    await writeFile(join(fixtures, "NOTE.md"), "Generated by [`generators/build.py`](./generators/build.py).\n");
    assert.doesNotThrow(() => runGate(root));
    await writeFile(join(fixtures, "NOTE.md"), "Generated by `elsewhere/build.py`.\n");
    const output = runGateExpectingFailure(root);
    assert.match(output, /generators\/build\.py: named in no provenance record/);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
...
test("a path row resolving outside its fixture root fails with its own message", async () => {
  ...
  try {
    ...
    await writeFile(join(root, "scripts/fixtures", "README.md"), "shared.bin\n");
    await writeFile(
      join(fixtures, "CAPTURE_PROVENANCE.md"),
