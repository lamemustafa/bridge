...
''')
    ...
    if phase == "writer":
        ...
        transport_hit = first["cache_hits"]["counts"].get("Rust", 0) > 0 and first["compilations"] == 0
        build("2-unwrapped-control", False)
        build("3-remote-repeat", True)
        ...
        execute(["sccache", "--stop-server"], "stop-server.log")
    summary = {"phase": phase, "transport_hit": transport_hit, "controls_complete": True, "candidate_correct": all(row["correct"] for row in rows),
