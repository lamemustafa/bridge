...
- On Git 2.43.0: 20 tests, 17 pass, 3 fail. The 3 failures are the item 3 merge-driver group only. Inverse mutations, one run each: dropping `--upload-pack` failed both owner tests. Trusting `*` failed the owner test at its wrong-trust control. Dropping the quoting failed only the quoting test. The Sonnet review found nothing blocking.
